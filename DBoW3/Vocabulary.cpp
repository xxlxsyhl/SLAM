...
NodeId Vocabulary::getNodeId(const cv::Mat &feature, const int level) const
{
    if(empty())
    {
        return 0;
...
//    DescManip::distance(feature, m_nodes[final_id].descriptor);
  ...
  //added by yhl
  double weight_dis = 1.0 - 1.0*(double)best_dis/(double)256;

  // turn node id into word id
  word_id = m_nodes[final_id].word_id;
  weight = m_nodes[final_id].weight*weight_dis;
}
