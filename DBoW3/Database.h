...
public:
   ...
   * @param right
   */
    int query(const cv::Mat &features, const int left, const int right);
  ...
  /**
   * save the database using binary
   * @param filename
   */
  void saveb(const std::string &filename) const;

  /**
   * load the database file binary
   * @param filename
   */
  void loadb(const std::string &filename);
